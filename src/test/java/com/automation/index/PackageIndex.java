...
public class PackageIndex extends SeleniumInit{
	...
	public void testMobile()
	{
		...
		packageVerification_mobile=packageIndexpage_mobile.checkMailinator(1);
	//	packageIndexpage_mobile.checkMailinator();
		
		Common.logstep("Step "+ (step++) +" Enter test email address.");
		packageIndexpage_mobile.entermailinatorEmail(PackageIndexpage.email);
		
		Common.logstep("Step "+ (step++) +" Click on email notiction received from 'ACME Business Banking'.");
		packageVerification_mobile=packageIndexpage_mobile.checkEmailContent();
		
	//	Common.log("Size of Window-->"+driver1.getWindowHandles().size());
				
		Common.logstep("Step "+ (step++) + " Click on 'Go to My documents' button to navigate Dashboard.");
		packageIndexpage_mobile.clickGoToDocument();
		
		Common.pause(20);

		
		/*ArrayList tabs = new ArrayList (driver1.getWindowHandles());
        System.out.println("Size after Click is " + tabs.size());*/
        
		/*String currentwindow = driver2.getWindowHandle();
        System.out.println(" Id of Current Tab is " + currentwindow);*/
        	

		Common.logstep("Step "+ (step++) +" Navigate to other tab and accept Consent.");
		packageIndexpage_mobile.clickAccept();
			
		Common.logstep("Step "+ (step++) +" Click on 'Request SMS Authentication Code' button.");
		packageIndexpage_mobile.clickRequestSMS();
		
		Common.logstep("Step "+ (step++) +" Enter SMS Code.");
		packageIndexpage_mobile.enterSMSCode();
	
		Common.logstep("Step "+ (step++) +" Click on Authenticate button to verify SMS Code.");
		packageIndexpage_mobile.clickAuthnticate();
			
		Common.logMandetoryAssert("Verify that user navigates to 'Documents Portal'.");
		if(packageVerification_mobile.verifyDocPortalpage("b"))
		{
			Common.AssertPassed();
			Assert.assertTrue(true);
		}
		...
		packageVerification_mobile=packageIndexpage_mobile.clickReturn();	
		
		Common.logMandetoryAssert("Verify that 'eSign documents' status changed to 'Signed'.");
		if(packageVerification_mobile.verifySignStatus())
		{
			Common.AssertPassed();
			Assert.assertTrue(true);
		}
	...
	public void demo()
	{
		...
		Common_demo.logcase(" TC_Demo : Verify the Demo Test Case");

		Common_demo.logstep("Step "+ (step++)+ " Enter Username");
		packageIndexpage_demo.enterUsername(TestData.getUserName());

		Common_demo.logstep("Step "+(step++)+ " Enter Password");
		packageIndexpage_demo.enterPassword(TestData.getPassword());

		Common_demo.logstep("Step "+(step++)+ " Click on Login BUtton ");
		packageVerification_demo = packageIndexpage_demo.clickLogin();
			
		Common_demo.logstep(" Verify the User Created ");
		if(packageVerification_demo.verifyUser())
		{
		...
		}else
		{
			...
			Common_demo.makescreenshot(driver, "LoginFailed");
		}	
		
		Common_demo.logstep("Step "+ (step++) +" Select workflow from dropdown.");
		packageIndexpage_demo.selectworkflow();
		
		Common_demo.logstep("Step "+(step++)+ "Click on Create button of Workflow" );
		packageVerification_demo = packageIndexpage_demo.ClickCreateWF();
		
		Common_demo.logstep("Step "+(step++)+ "Enter Borrower's Details" );
		packageIndexpage_demo.referenceName();
		packageIndexpage_demo.enterFirstName();
		packageIndexpage_demo.enterLastName();
		packageIndexpage_demo.enterEmail();
		packageIndexpage_demo.enterMobile();
		
		Common_demo.logstep(" Select the attachments to be done ");
		packageIndexpage_demo.chooseattachments();
		
		Common_demo.logstep(" Click on Create Package ");
		packageVerification_demo = packageIndexpage_demo.clickCreatepkg();
		
		Common_demo.logstep(" Click on Choose Service Checkbox ");
		packageIndexpage_demo.CloudServicechkbox();
		
		Common_demo.logstep(" Select the Cloud Service Agreement " );
		packageIndexpage_demo.uploadCloudAgreement();
		
		Common_demo.logstep(" Click on 'Send Now' button " );
		packageVerification_demo = packageIndexpage_demo.ClickSendNow();
		
		Common_demo.logstep(" Verify the packet created successfully and it is visible in Dashboard ");
		//Common_demo.logcase(packageVeri);
		if(packageVerification_demo.verifyPkg())
		{
		...
		{
			Assert.assertTrue(false);
			System.out.println(" Package is not created successfully ");
			Common_demo.makescreenshot(driver, "LoginFailed");
		}	
		
		Common_demo.logcase("Enter Mailinator");
		packageVerification_demo=packageIndexpage_demo.OpenMailinatorURL();

		Common_demo.logcase(" Enter Email Address ");
		packageIndexpage_demo.entermailinator(packageIndexpage_demo.BrwMail);
