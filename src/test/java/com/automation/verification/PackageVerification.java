...
public class PackageVerification extends AbstractPage{
	...
	public boolean verifyPkgComplete(String email, String string) {
		...
		String emailSubject="//*[contains(@class,'ng-binding')][contains(text(),'Package complete, reference')]";
	...
	public boolean verifyEmail() {
		// TODO Auto-generated method stub
		String emailSubject="//*[contains(@class,'ng-binding')][contains(text(),'"+PackageIndexpage.reference+"')]";
		for(int i=0;i<7;i++)
		{
			if(Common.waitForElementAndRefresh(driver, emailSubject)) {
				Common.pause(2);
				return driver.findElement(By.xpath("//*[contains(@class,'ng-binding')][contains(text(),'"+PackageIndexpage.reference+"')]")).isDisplayed();
			}
		}
		return false;
	}
